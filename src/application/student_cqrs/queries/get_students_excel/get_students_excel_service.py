...
class GetStudentsExcelService(AbstractGetStudentsExcelService):
    ...
    def get_students(self):
        ...
        string_dest_path = os.path.join(destination_path, "src", "application", "stored_files", "hello.xlsx")
        dest_path = Path(string_dest_path)
        students_dataframe.to_excel(dest_path)

        '''we are returning the excel file that we created and created a background task so our file is deleted on our server after it has been returned
        ...
        file_res = FileResponse(path=string_dest_path, filename=string_dest_path,
                                background=dest_path.unlink and print("File " + string_name + " removed"))
